  ...
  withPostgresServer(
    ...
    (server) {
      ...
      setUpAll(() async {
        ...
        // stream
        changesConn = await Connection.open(await server.endpoint());
        ...
        final statement =
            'START_REPLICATION SLOT $slotName LOGICAL $xlogpos '
      ...
      test('- Receive InsertMessage after insert statement', () async {
        ...
        controller = StreamController(
          onListen: () async {
            ...
            await changesConn.execute(
              "insert into $changesTable (value) values ('test');",
            );
            await future;
      ...
      test('- Receive UpdateMessage after update statement', () async {
        ...
        await changesConn.execute(
          "insert into $changesTable (value) values ('update_test');",
        );
      ...
      test('- Receive TruncateMessage after delete statement', () async {
        ...
        controller = StreamController(
          onListen: () async {
            // don't await here otherwise what's after won't be executed.
            final future = controller.addStream(stream);
            await changesConn.execute('truncate table $truncateTable;');
            await future;
            await controller.close();
          },
        );
