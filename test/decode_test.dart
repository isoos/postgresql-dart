...
void main() {
  withPostgresServer('simple query protocol decode', (server) {
    ...
    test('timestamp without time zones', () async {
      final rs = await conn.execute(
        "SELECT '1999-01-08 04:05:06'::TIMESTAMP WITHOUT TIME ZONE",
      );
      final item = rs.single.single as DateTime;
    ...
    test('interval', () async {
      ...
      expect(
        (item as UndecodedBytes).asString,
  ...
  withPostgresServer('function', (server) {
    test('function returns select', () async {
      ...
      await c.execute(
        'CREATE OR REPLACE FUNCTION get_t1_data() RETURNS '
  ...
  withPostgresServer('enum', (server) {
    test('custom type enum as string', () async {
      ...
      await c.execute(
        r'''INSERT INTO portfolio_item values ($1, $2)''',
  ...
  withPostgresServer('sql null', (server) {
    test('decode output', () async {
      final c = await server.newConnection();
      final rs = await c.execute("SELECT NULL, 'null'::JSONB, NULL::JSONB, 1");
      final row = rs.single;

      expect(row[0], isNull);
      expect(row.isSqlNull(0), isTrue);
      expect(row[1], isNull);
      expect(row.isSqlNull(1), isFalse);
      expect(row[2], isNull);
      expect(row.isSqlNull(2), isTrue);
      expect(row[3], isNotNull);
      expect(row.isSqlNull(3), isFalse);
    });
  });
}
